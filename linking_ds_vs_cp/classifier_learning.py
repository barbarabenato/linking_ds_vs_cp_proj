...
def vgg16_classification(imgs, labels, class_learn_params, file_name): 
    batch, epochs, n_classes = class_learn_params
    ...
    # adding classification layers after the backbone's layers
    ...
    opt = optimizers.SGD(lr=1e-1, decay=1e-6, momentum=0.9, nesterov=True)
    model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])
    history = model.fit(imgs, to_categorical(labels,n_classes), batch_size=batch, epochs=epochs, verbose=0, validation_split=0.2, shuffle=True, callbacks=callbacks)
